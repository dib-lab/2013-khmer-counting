...
tradeoff is to collect k-mer counts on disk, and merge counts as an
offline task, where the k-mer counts for that data set are not available
...
in memory, necessary to support streaming k-mer analysis algorithms.
...
moreover, k-mers cannot be enumerated directly from the database.
Here we analyze the speed, memory usage, and miscount rate of khmer
...
efficient tool for online k-mer counting in DNA sequence.  In
particular, the miscount behavior of the CountMin Sketch performs well
...
for each fixed-length DNA word of length k in a dataset
...
k-mers and contain many erroneous k-mers.  The data sets are sparse
because for typical values of k such as k=32, only a small fraction of
the total possible number of k-mers ($4^{32}$) are actually present in
the data set.  The high error rate (e.g. Illumina has a ~0.1-1\%
per-base error rate \cite{pubmed19997069}) generates many unique k-mers.  In
particular, as the total number of reads generated increases, the
total number of errors grows linearly, leading to data sets where the
...
Moreover, we wish to enable the use of cloud computing and desktop
...
enumeration of k-mers directly from the data structure; instead,
...
Below, we compare time, memory and disk usage of our k-mer counting
implementation with that of Tallymer, Jellyfish, and DSK, for two
problems: first, generating a k-mer abundance distribution for large
...
will never falsely call a high-abundance k-mer as low-abundance.
...
above, choosing CountMin Sketch sizes yielding three different false
