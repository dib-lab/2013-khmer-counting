...
\author{Qingpeng Zhang, Jason Pell, Rosangela Canino-Koning, Adina Chuang Howe, and C. Titus Brown}
\begin{document}
...
results (@cite - KmerGenie?). K-mer counting can also give important information for
...
true k-mers \cite{ConwayBromage}.  Dealing with the resulting large numbers of k-mers, most of which
...
includes Tallymer\cite{Kurtz2008}, Jellyfish\cite{Marcais2011}, BF-Counter\cite{Melsted2011}, DSK\cite{Rizk2013}, KMC\cite{Deorowicz2013}, and Turtle (@cite).
...
In addition, the memory usage of khmer also increases
...
with the desired false positive rate: we can decrease the memory usage by increasing
the false positive rate, as shown in Figure \ref{cmp_memory}.  We can also
see from the figure that with a low counting error rate of 1\%, the
memory usage is competitive with other programs.
...
reuse; thus the uncompressed disk usage for khmer in Figure \ref{cmp_disk} is the
same as its memory.
...
reference genomes (@CTB for example...? Quake?).  This is an application where the counting error
...
% @CTB are you using filter-abund? That trims, not removes. QP?
...
% @CTB move this last sentence into discussion?
...
time requirements considerably in follow-on assembly (@cite adina/large metagenomes).
...
errors from collison will not influence the count of these unique
...
@CTB mention why we disqualify DSK
...
the approach taken by DSK @cite.
...
Jellyfish is version 1.1.2 and the multithreading option is off. @CTB
...
\caption{Time usage of different khmer counting tools. @CTB x axis:reads?}
\label{cmp_time}
\end{figure}
