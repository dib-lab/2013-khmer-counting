...
existing techniques, the CountMin Sketch does store k-mers directly;
instead,
...
We compare time, memory, and disk usage of our k-mer counting
...
common analysis approach for determing assembly parameters \cite{ChikhiXXX}.
%@CTB
...
of k-mers being counted increases.  For a 5 GB data set with 2.7
...
usage of 40 MB.  In the first round, the estimated error
...
the fourth iteration the error rate was down to 3.1\% with 0.0\% of the data removed (Table @XXX.).
...
are never counted.  This has the advantage of enabling low-memory
...
k-mers even at the highest error rate (column 3), while discarding only
...
especially metagenomic data sets --- high memory demands constrain analysis \cite{adina2013}.  Moreover, because the error
...
required to achieve a error rate of 1\% for $N \approx
...
Two human gut metagenome reads data sets (MH0001 and MH0002) were used from the 
MetaHIT (Metagenomics of the Human Intestinal Tract) project\cite{Qin2010}. 
The MH0001 data set contains approximately 59 million reads, each 44bp long. 
The MH0002 data set consists of about 61 million 75bp long reads.
Each FASTA file was trimmed to remove low quality sequences. 

Five soil metagenomics reads data sets with different size were taken
from GPGC project for benchmark purpose.
(@CTB Iowa Prairie Table 1\ to cite here.)
...
\caption{Time for different k-mer counting tools to retrieve the counts of 9.7m randomly chosen k-mers (y axis), plotted against the number of unique k-mers in the data set being queried (x axis).  DSK does not support this functionality.}
\label{cmp_count}
\end{figure}
